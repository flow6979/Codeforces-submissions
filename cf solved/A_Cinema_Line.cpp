...
int main(){
    int n,csh25=1,csh50=0;
    bool check=true;
    cin>>n;
    int v[n];
    f(i,0,n){cin>>v[i];}
    if(v[0]!=25){prnt("NO");check=false;}
    else{
        f(i,1,n){
            // if(csh25<=0){prnt("NO");check=false;break;}       
            if(v[i]==25){csh25++;}
            else if(v[i]==50){
                if(csh25>=1){csh50++;csh25--;}
                else{prnt("NO");check=false;break;}
            }
            else if(v[i]==100){
            if(csh50>=1 && csh25>=1){csh25--;csh50--;}
            else if(csh50<1 && csh25>=3){csh25-=3;}
            else{prnt("NO");check=false;break;}
            }
            // cout<<r<<" ";
        }
