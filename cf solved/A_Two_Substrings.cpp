...
int main(){
    ...
    cin>>s;
    vi v,a;
    f(i,0,s.size()-1){if(s[i]=='A' && s[i+1]=='B'){p=i+1;break;}}
    f(i,p,s.size()-1){if(s[i]=='B' && s[i+1]=='A'){k=i;}}

    f(i,0,s.size()-1){if(s[i]=='B' && s[i+1]=='A'){t=i+1;break;}}
    f(i,t,s.size()-1){if(s[i]=='A' && s[i+1]=='B'){n=i;}}

    if(p==0 || t==0){cout<<"NO";}
    else if(s.size()>3 && p<k){cout<< "YES";}
    else if(s.size()>3 && n>t){cout<< "YES";}
    else {cout<<"NO";}
}