...
int main(){
    tc{
        ...
        vi p;
        f(i,0,n){cin>>v[i];}
        f(i,0,n){if(v[i]<0){v[i]=-v[i];}}
        sort(v,v+n);
        f(i,0,n){if(v[i]==0)t++;}
        f(i,0,n){if(v[i]!=v[i+1])p.pb(v[i]);}
        f(i,0,p.size()){f(j,0,n){if(p[i]==v[j] && p[i]!=0){k++;}}
            if(k>1){m++;} k=0;}
        if(t>0){cout<<m*2+(p.size()-m-1)+1<<endl;}
        else {cout<< m*2+(p.size()-m)<<endl;}
    }
}