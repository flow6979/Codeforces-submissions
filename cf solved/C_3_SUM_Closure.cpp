...
int main(){
    tc{
        ...
        if(p>2 || pp>2)cout<<"NO"<<endl;
        else{
            ll cntz=0;
            f(i,0,n){if(v[i]==0)cntz++;}
            if(cntz<2){
                f(i,0,n){
                    f(j,i+1,n){
                        f(k,j+1,n){
                            if(s.find(v[i]+v[j]+v[k])==s.end()){chk=1;break;}
                        }
                        if(chk==1)break;
                    }
                    if(chk==1)break;
                ...
                vl vv;
                f(i,0,n){if(v[i]!=0)vv.pb(v[i]);}
                vv.pb(0);
                set<ll> ss;
                ll nn=vv.size();
                f(i,0,nn){ss.insert(vv[i]);}
                f(i,0,nn){
                    f(j,i+1,nn){
                        f(k,j+1,nn){
                            if(ss.find(vv[i]+vv[j]+vv[k])==ss.end()){chk=1;break;}
                        }
                        if(chk==1)break;
                    }
                    if(chk==1)break;
