...
int main(){
    tc{
        ...
        cin>>a>>b;
        char v[a][a];
        f(i,0,a){f(j,0,a){v[i][j]='.';}}
        if(b<=(a/2)+1 && a%2!=0){f(i,0,b){v[i*2][i*2]='R';}f(i,0,a){f(j,0,a){cout<<v[i][j];}cout<<endl;} }
        else if(b<=(a/2) && a%2==0){f(i,0,b){v[i*2][i*2]='R';}f(i,0,a){f(j,0,a){cout<<v[i][j];}cout<<endl;} }
        else{cout<<-1<<endl;}
