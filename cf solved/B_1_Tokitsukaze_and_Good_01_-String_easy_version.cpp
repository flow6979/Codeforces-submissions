...
#define decide(cond)   auto result = (cond) ? "YES" : "N0";cout<<result<<endl;
...
ll mod_string(string num, ll a){ll res = 0; for (ll i = 0; i < num.length(); i++)res = (res * 10 + (int)num[i] - '0') % a;return res;}
...
//Bits
string decToBin(ll n){string s="";ll i = 0;while (n > 0) {s =to_string(n % 2)+s;n = n / 2;i++;}return s;}
...
int main(){
    tc{
        int n,k=0,r=0,check=0,cnt=0,val=0,t=1;
        cin>>n;
        string s;
        cin>>s;
        vi v,vv;
        f(i,0,n-1){if(s[i]!=s[i+1]){r=i+1;v.pb(r-k);k=r;}}
        v.pb(n-r);
        f(i,0,v.size()){if(v[i]%2==1){check=1;}}
        if(check==0){cout<<0<<endl;}
        else{
            f(i,0,n-1){
                if(s[i]==s[i+1]){t++;}
            }
            if(t==n && n%2==0){cout<<0<<endl;}
            else if(t==n && n%2==1){cout<<n<<endl;}
            else{
            f(i,0,v.size()){
                if(v[i]%2==1){cnt++;}
            }
            if(cnt%2==0){
                ...
                f(i,0,vv.size()){val+=(vv[i+1]-vv[i]);i+=1;}
            }
            cout<<val<<endl;
            }
        }
