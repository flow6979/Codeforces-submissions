...
#define prnt(a)          cout<<"["; for(auto p:a) cout<<p<<" "; cout<<"]"<<endl;
#define op(prnt)         cout<<prnt<<endl
...
int main(){
    tc{
        ...
        f(i,1,q){
            f(j,1,w){
            if(v[i][j]==v[i-1][j-1] && v[i-1][j]!=v[i][j-1] && v[i][j]==1 && u==0){op("NO");u=1;break;}
            else if(v[i][j]!=v[i-1][j-1] && v[i-1][j]==v[i][j-1] && v[i][j-1]==1 && u==0){op("NO");u=1;break;}
            }
        }
        if(u==0){op("YES");}
    }
