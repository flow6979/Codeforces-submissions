...
ll factorial(ll n) {ll factorial = 1;for (ll i = 2; i <= n; i++)factorial = factorial * i;return factorial;}  //O(n)
...
int main(){
    ...
    f(i,0,n){cin>>v[i];}
    sort(v,v+n);
    ll ans=v[n-1]-v[0],l=1,h=1;
    if(v[0]==v[n-1]){cout<<ans<<" "<<n*(n-1)/2;}
    else{
        f(i,1,n){if(v[i]==v[0]){l++;}
                else{break;}}
        fd(i,n-2,0){if(v[i]==v[n-1]){h++;}
                else{break;}}
