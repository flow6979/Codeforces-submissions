...
int main(){
    ...
    f(i,0,n){cin>>v[i];}
    sort(v,v+n);
    f(i,0,n){if(v[i]!=v[i+1]) k++;}
    if(k>3){cout<<-1;}
    else{
        if(n==1){cout<<0;}
        else if(n>1){
        f(i,0,n){if(v[i]-v[0]!=0){s=v[i]-v[0];break;}}
        f(i,0,n-1){if(v[i+1]-v[i]==0 || v[i+1]-v[i]==s)g++;}
        if(g==n-1 && (v[n-1]-v[0])%2!=0 ){cout<<s;}
        else if(g==n-1 && (v[n-1]-v[0])%2==0 ){cout<<(v[n-1]-v[0])/2;}
        else{cout<<-1;}}
    }
}
