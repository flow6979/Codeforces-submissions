...
int main(){
    tc{
        ...
        cin>>s;
        // asc(s);
        f(i,0,s.size()){p+=(s[i]-'a'+1);}
        if(s.size()%2==0){cout<<"Alice"<<" "<<p<<endl;}
        else if(s.size()==1){cout<<"Bob "<<s[0]-'a'+1<<endl;}
        else{
            if(s[0]<s[s.size()-1]){cout<<"Alice "<<p-2*(s[0]-'a'+1)<<endl;}
            else{cout<<"Alice "<<p-2*(s[s.size()-1]-'a'+1)<<endl;}
        }
    }
}