...
int main(){
    ...
    f(i,0,v.size()){
        if(v[i]%2==0 && v[i]<v[v.size()-1]){cnt=1;swap(v[i],v[v.size()-1]);break;}
    }
    f(i,0,v.size()){
        if(v[v.size()-1-i]%2==0 && cnt==0 && v[v.size()-1-i]>v[v.size()-1]){cnt=1;swap(v[v.size()-1-i],v[v.size()-1]);break;}
    }
    if(cnt==0){cout<<-1;}
    else{
        f(i,0,v.size()){ans+=v[i];}
        cout<<ans;
    }
}